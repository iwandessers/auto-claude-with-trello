...
1. NORMAL TASK FLOW  (python auto-claude-with-trello.py [--loop])
-----------------------------------------------------------------------------
...
2. ORCHESTRATOR FLOW  (python auto-claude-with-trello.py orchestrate --card-id ID)
...
3. ORCHESTRATOR SUB-AGENT FLOW  (runs inside ProcessPoolExecutor)
-----------------------------------------------------------------------------

    Each sub-agent is a separate OS process in its own git worktree.
    Multiple sub-agents run in parallel (up to --max-agents).
